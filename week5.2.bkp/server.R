...
shinyServer(function(input, output, session){
  ...
  output$words <- renderUI({
    ...
    cat(predictCount)
    valueT <- modelpred()
    predicWord <- getNextTerm(valueT, predictCount)
    ...
    cat(paste(predictCount,"testing"))
    ...
    buttons <- lapply(buttons, function(i) {
      ...
      if (is.null(obsList[[btName]])) {
        ...
        obsList[[btName]] <<- observeEvent(input[[btName]], {
          cat("Button ", i, "\n")
          updateTextAreaInput(session, "textArea", value = paste(valueT, btName))
        })
      }
      actionButton(btName,paste(predicWord[i,1]))
      ####
    }
    )
  })
