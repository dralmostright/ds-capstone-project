...
shinyServer(function(input, output, session){
  ...
  getNextTerm <- function(findTerm, clusterSize){
    ...
    while(stringLen >= 0) {
      ...
      ## if no match found on any n-gram model randomly select 5 words from uniGram
        ...
        predicWord <- head(uniTFM[,2:1],clusterSize)
  ...
  output$words <- renderUI({
    ...
    buttons <- as.list(1:length(predicWord[,2]))
    buttons <- lapply(buttons, function(i) {
        actionButton(paste0(predicWord[i,2],i),paste(predicWord[i,2]))
    }
    )
  })
  ...
  observeEvent(input$predictWord, {
    predictCount <- input$wordCloud
    predicWord <<- getNextTerm(modelpred(), predictCount)
    cat("I am clicked..")
    cat(str(predicWord))
    })
    #predicWord <- getNextTerm(modelpred(), input$wordCloud)
  ##})
  ...
  output$plot1 <- renderPlot({
    ...
    predicWord <<- predicWord[1:predictCount,]
    wordcloud(predicWord[,2], predicWord[,1], colors=brewer.pal(8, "Dark2"), random.order=FALSE, rot.per=0.5 )
